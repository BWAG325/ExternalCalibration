...
// CornerDetAC::CornerDetAC() = default;
CornerDetAC::~CornerDetAC() = default;
...
CornerDetAC::CornerDetAC() {
	...
	templateProps.emplace_back(static_cast<dtype>(0), static_cast<dtype>(CV_PI / 2));
	templateProps.emplace_back(static_cast<dtype>(CV_PI / 4), static_cast<dtype>(-CV_PI / 4));
	templateProps.emplace_back(static_cast<dtype>(0), static_cast<dtype>(CV_PI / 2));
	templateProps.emplace_back(static_cast<dtype>(CV_PI / 4), static_cast<dtype>(-CV_PI / 4));
	templateProps.emplace_back(static_cast<dtype>(0), static_cast<dtype>(CV_PI / 2));
	templateProps.emplace_back(static_cast<dtype>(CV_PI / 4), static_cast<dtype>(-CV_PI / 4));
...
dtype CornerDetAC::normalPDF(dtype dist, dtype mu, dtype sigma) {
...
//*************************************************************************//
void CornerDetAC::createKernel(float angle1, float angle2, int kernelSize, Mat& kernelA, Mat& kernelB, Mat& kernelC,
                               Mat& kernelD) {
	...
	for (int u = 0; u < width; ++u) {
		for (int v = 0; v < height; ++v) {
			dtype vec[] = {static_cast<dtype>(u - kernelSize), static_cast<dtype>(v - kernelSize)}; //相当于将坐标原点移动到核中心
			...
			if (side1 <= -0.1 && side2 <= -0.1) {
				kernelA.ptr<dtype>(v)[u] = normalPDF(dis, 0, kernelSize / 2);
			}
			if (side1 >= 0.1 && side2 >= 0.1) {
				kernelB.ptr<dtype>(v)[u] = normalPDF(dis, 0, kernelSize / 2);
			}
			if (side1 <= -0.1 && side2 >= 0.1) {
				kernelC.ptr<dtype>(v)[u] = normalPDF(dis, 0, kernelSize / 2);
			}
			if (side1 >= 0.1 && side2 <= -0.1) {
				kernelD.ptr<dtype>(v)[u] = normalPDF(dis, 0, kernelSize / 2);
			}
		}
	}
...
void CornerDetAC::getMin(Mat src1, Mat src2, Mat& dst) {
	...
	for (int i = 0; i < nr; i++) {
		...
		auto* dataResult = dst.ptr<dtype>(i);
...
//获取梯度角度和权重
void CornerDetAC::getImageAngleAndWeight(const Mat& img, Mat& imgDu, Mat& imgDv, Mat& imgAngle, Mat& imgWeight) {
	...
	for (int i = 0; i < imgDu.rows; i++) {
		for (int j = 0; j < imgDu.cols; j++) {
			auto* dataAngle = imgAngle.ptr<dtype>(i);
...
//*************************************************************************//
void CornerDetAC::nonMaximumSuppression(Mat& inputCorners, vector<Point2f>& outputCorners, int patchSize,
                                        dtype threshold, int margin) {
	if (*inputCorners.size <= 0) {
		cout << "The input mat is empty!" << endl;
		return;
	}
		...
		for (int j = margin + patchSize; j <= inputCorners.rows - (margin + patchSize + 1); j = j + patchSize + 1) {
			...
			for (int m = maxX - patchSize; m <= min(maxX + patchSize, inputCorners.cols - margin - 1); m++) //二次检查
			{
				for (int n = maxY - patchSize; n <= min(maxY + patchSize, inputCorners.rows - margin - 1); n++) {
					if (inputCorners.ptr<dtype>(n)[m] > maxVal && (m < i || m > i + patchSize || n < j || n > j +
						patchSize)) {
						flag = 1;
						break;
					}
...
void CornerDetAC::findModesMeanShift(vector<dtype> hist, vector<dtype>& hist_smoothed, vector<pair<dtype, int>>& modes,
                                     dtype sigma) {
	...
	for (int i = 0; i < hist.size(); i++) {
		dtype sum = 0;
		for (int j = -static_cast<int>(round(2 * sigma)); j <= static_cast<int>(round(2 * sigma)); j++) {
			int idx = 0;
			idx = (i + j) % hist.size();
			sum = sum + hist[idx] * normalPDF(j, 0, sigma);
		}
		hist_smoothed[i] = sum;
		if (abs(hist_smoothed[i] - hist_smoothed[0]) > 0.0001)allZeros = false;
		// check if at least one entry is non-zero
		//(otherwise mode finding may run infinity)
	}
	if (allZeros)return;
	...
	for (int i = 0; i < hist.size(); i++) {
		...
		if (ys == true) {
			modes.emplace_back(hist_smoothed[j], j);
		}
	}
...
void CornerDetAC::edgeOrientations(Mat imgAngle, Mat imgWeight, int index) {
	...
	for (int i = 0; i < vec_angle.size(); i++) {
		int bin = max(min(static_cast<int>(floor(vec_angle[i] / pin)), binNum - 1), 0);
...
//亚像素精度找角点
void CornerDetAC::refineCorners(vector<Point2f>& corners, Mat imgDu, Mat imgDv, const Mat& imgAngle,
                                const Mat& imgWeight, float radius) {
	// image dimensions
		...
		for (int u = startX; u < startX + ROIwidth; u++)
			for (int v = startY; v < startY + ROIheight; v++) {
				...
				if (u != cu || v != cv) // % do not consider center pixel
				{
					...
					if ((d1 < 3) && abs(o.x * cornersEdge1[i][0] + o.y * cornersEdge1[i][1]) < 0.25 || (d2 < 3) && abs(
						o.x * cornersEdge2[i][0] + o.y * cornersEdge2[i][1]) < 0.25) {
...
void CornerDetAC::cornerCorrelationScore(const Mat& img, Mat imgWeight, vector<Point2f> cornersEdge, float& score) {
	//center
	...
	for (int i = 0; i < imgWeight.cols; i++) {
		for (int j = 0; j < imgWeight.rows; j++) {
			imgWeight.ptr<dtype>(j)[i] = static_cast<dtype>(imgWeight.ptr<dtype>(j)[i] - mean.ptr<double>(0)[0]) /
				static_cast<dtype>(std.ptr<double>(0)[0]);
			img_filter.ptr<dtype>(j)[i] = static_cast<dtype>(img_filter.ptr<dtype>(j)[i] - mean1.ptr<double>(0)[0]) /
				static_cast<dtype>(std1.ptr<double>(0)[0]);
		}
	}
	...
	sum = (dtype)sum / static_cast<dtype>(vec_weight.size() - 1);
	dtype score_gradient = sum >= 0 ? sum : 0;

	//create intensity filter kernel
	Mat kernelA, kernelB, kernelC, kernelD;
	createKernel(atan2(cornersEdge[0].y, cornersEdge[0].x), atan2(cornersEdge[1].y, cornersEdge[1].x), c[0], kernelA,
	             kernelB, kernelC, kernelD); //1.1 产生四种核
...
//score corners
void CornerDetAC::scoreCorners(const Mat& img, const Mat& imgAngle, const Mat& imgWeight, vector<Point2f>& corners,
                               vector<int> radius, vector<float>& score) {
	//for all corners do
	for (int i = 0; i < corners.size(); i++) {
		//corner location
		...
		for (int j = 0; j < radius.size(); j++) {
			...
			if (u > r && u <= (img.cols - r - 1) && v > r && v <= (img.rows - r - 1)) {
				...
				cornersEdge.emplace_back((float)cornersEdge1[i][0], (float)cornersEdge1[i][1]);
				cornersEdge.emplace_back((float)cornersEdge2[i][0], (float)cornersEdge2[i][1]);
...
void CornerDetAC::detectCorners(Mat& Src,  Corners& mCorners, dtype scoreThreshold,
                                bool isRefine) {
	Mat gray, imageNorm;
	...
	for (int i = 0; i < 6; i++) {
		...
		createKernel(templateProps[i].x, templateProps[i].y, radius[i / 2], kernelA1, kernelB1, kernelC1,
		             kernelD1); //1.1 产生四种核
	...
	if (nlen > 0) {
		for (int i = 0; i < nlen; i++) {
			if (score[i] > scoreThreshold) {
				...
				mCorners.v1.emplace_back(cornersEdge1[i][0], cornersEdge1[i][1]);
				mCorners.v2.emplace_back(cornersEdge2[i][0], cornersEdge2[i][1]);
