...
ChessboardStruct::ChessboardStruct() = default;
...
void ChessboardStruct::directionalNeighbor(int idx, cv::Vec2f v, cv::Mat chessboard, Corners& corners,
                                           int& neighbor_idx, float& min_dist) {
#if 1
...
cv::Mat ChessboardStruct::initChessboard(Corners& corners, int idx) {
	...
	sigood = sigood || (dist2[0] < 0) || (dist2[1] < 0) || (dist2[2] < 0) || (dist2[3] < 0) || (dist2[4] < 0) || (dist2[
		5] < 0);
...
float ChessboardStruct::chessboardEnergy(cv::Mat chessboard, Corners& corners) const {
	float lamda = m_lamda;
	...
	for (int i = 0; i < chessboard.rows; i++)
		for (int j = 0; j < chessboard.cols - 2; j++) {
			...
			if (E_structure < E_structure0) E_structure = E_structure0;
		}
...
void ChessboardStruct::predictCorners(std::vector<cv::Vec2f>& p1, std::vector<cv::Vec2f>& p2,
...
void ChessboardStruct::assignClosestCorners(std::vector<cv::Vec2f>& cand, std::vector<cv::Vec2f>& pred,
                                            std::vector<int>& idx) {
...
cv::Mat ChessboardStruct::growChessboard(cv::Mat chessboard, Corners& corners, int border_type) {
		...
		for (int row = 0; row < chessboard.rows; row++)
			for (int col = 0; col < chessboard.cols; col++) {
				if (col == chessboard.cols - 3) {
					...
					p1.emplace_back(p[ij]);
				}
				if (col == chessboard.cols - 2) {
					int ij = chessboard.at<int>(row, col);
					p2.emplace_back(p[ij]);
				}
				if (col == chessboard.cols - 1) {
					int ij = chessboard.at<int>(row, col);
					p3.emplace_back(p[ij]);
				}
			}
		std::vector<int> idx;
...
void ChessboardStruct::chessboardsFromCorners(Corners& corners, std::vector<cv::Mat>& chessboards, float lamda) {
	logd
	("Structure recovery:\n");
	m_lamda = lamda;
	for (int i = 0; i < corners.p.size(); i++) {
		if (i % 128 == 0) std::cout << i << "/" << corners.p.size() << std::endl;
		...
		while (true) {
			...
			// accept the best proposal, if energy is reduced
			cv::Mat chessboardt;
			...
			if (isoverlap == false) {
				...
				for (int i1 = 0; i1 < overlap.rows; i1++) {
					if (fabs(overlap.at<float>(i1, 0)) > 0.0001) // ==1//ÓÐÖØµþ
					{
						...
						bool isb2 = a > b;
						if (isb1 != isb2) std::cout << "find bug!" << std::endl;
						if (isb2) {
							flagerase[i1] = true;
						} else {
							flagpush = false;
...
void ChessboardStruct::drawChessboard(const cv::Mat& img, Corners& corners, std::vector<cv::Mat>& chessboards,
                                      const char* title, int t_, const cv::Rect rect) {
	...
	if (disp.channels() < 3) cv::cvtColor(disp, disp, cv::COLOR_GRAY2BGR);
	float scale = 0.3;
	int n = 8;
	...
	//为不同的棋盘生成不同的颜色
	cv::RNG rng(0xFFFFFFFF); //随机数生成，种子固定
