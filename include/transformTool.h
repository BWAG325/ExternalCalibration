...
class TransformTool {
    ...
    static static bool boardClassify(const Corners& corners, const std::vector<Chessboard>& boards,
    ...
    //四元数转化
    cv::Vec4f rotationMatrixToQuaternion(const cv::Mat& R);
...
public:
    ...
    static static bool lookupTransform(const std::vector<std::pair<cv::Mat, cv::Mat>>& maskTF, int main, int rest,
                                       std::pair<cv::Mat, cv::Mat>& transform);
    //在已经建立了主板后，后期添加其他的板的处理函数
    static static bool tfADD(std::pair<cv::Mat, cv::Mat>& transform, std::vector<std::pair<cv::Mat, cv::Mat>>& maskTF,
                             int main, int rest);

    //对旋转矩阵转化成4元数，使用opencv平均后，再转换回来，平移矩阵直接算术平均
